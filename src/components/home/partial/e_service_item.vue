<template>
  <div class='flex flex-col rounded-lg shadow-xl overflow-hidden'>
    ...
    <div class='flex-1 bg-white p-6 flex flex-col justify-between'>
      <div class='flex-1'>
        <router-link :to='{name:"EProvider", params:{ id:eService.e_provider.id }}' class='block mt-2'>
          <p class='text-xl font-semibold text-second-color-600'>
            {{ this.$filters.transString(eService.e_provider.name) }}
          </p>
      ...
      <div class='flex items-end justify-between'>
        ...
        <div class='flex flex-col pt-1 items-end  sm:pt-3'>
          <router-link :to='{name:"EProvider", params:{ id:eService.e_provider.id }}'
