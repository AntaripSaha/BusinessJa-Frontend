<template>
    <div class='relative pb-32 bg-accent-color-50 pt-5'>
      <div class='flex justify-between items-center px-4 pt-6 pb-16 mx-auto max-w-7xl sm:px-6 lg:px-8'>
        <div class='flex-1 min-w-0'>
          <h2 class='py-3 text-xl font-bold lg:text-4xl text-second-color-600'>Real Estate</h2>
    ...
    <section aria-labelledby='contact-heading'
      ...
      <div class='grid grid-cols-1 gap-y-20 lg:grid-cols-4 lg:gap-y-12 lg:gap-x-8'>
        <provider v-for="providers in realestateProvidersList" :key="providers.id" :provider='providers' />
      </div>
    </section>
  </template>
  ...
  <script>
    ...
    mounted() {
        this.getRealEstateProviderList()
    },
    computed: {
        ...mapState(['realestateProvidersList']),
    },
    methods: {
        ...mapActions(['getRealEstateProviderList']),
    },
    }
