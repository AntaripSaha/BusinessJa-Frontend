<template>
        ...
        <!-- <el-alert title="success alert" type="success" effect="dark" /> -->
            <div class='mt-2 divide-y divide-gray-100'>
                ...
                <div class="form newtopic">
                    ...
                    <button
                        class='mt-3 flex justify-center px-4 py-2 text-sm font-medium text-white rounded-md border border-transparent shadow-sm bg-main-color-600 hover:bg-main-color-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-main-color-500' v-bind:aria-required="true" @click="errorMsg">
                        {{ $t('Save') }}
                    </button>
                </div>
            </div>
  ...
  export default {
    ...
    errorMsg(response){
        ...
        this.toggleSnackBarAction(response);
    },
     ...
     saveRate(){
        ...
        response = axios.post('https://admin.yellowpageja.com/api/e_provider_reviews', this.posts).then(response => {
